...
BOOL CArpAppDlg::OnInitDialog()
{
	...
	srt_CList.InsertItem(0, _T("1"));   // Ï≤´Ïß∏Ìñâ(0), Ï≤´Ïß∏Ïó¥Ïóê ÏÇΩÏûÖ
	srt_CList.SetItem(0, 1, LVIF_TEXT, _T("NetMask"), NULL, NULL, NULL, NULL);   // Ï≤´Ïß∏Ìñâ(0), ÎëòÏß∏Ïó¥(1)Ïóê ÏÇΩÏûÖ 

	srt_CList.InsertItem(1, _T("2"));   // ÎëòÏß∏Ìñâ(1), Ï≤´Ïß∏Ïó¥Ïóê ÏÇΩÏûÖ
	srt_CList.SetItem(1, 1, LVIF_TEXT, _T("33535"), NULL, NULL, NULL, NULL);   // ÎëòÏß∏Ìñâ(1), ÎëòÏß∏Ïó¥(1)Ïóê ÏÇΩÏûÖ 
...
void CArpAppDlg::SetDlgState(int state) // ?§Ïù¥?ºÎ°úÍ∑?Ï¥àÍ∏∞??Î∂ÄÎ∂?
{
	UpdateData(TRUE);
	int i;
	...
	switch (state)
	{
	case IPC_INITIALIZING: // Ï≤??îÎ©¥ ?∏ÌåÖ
		pSendButton->EnableWindow(FALSE);
		m_ListArpTable.EnableWindow(TRUE);
		break;
	case IPC_READYTOSEND: // Send(S)Î≤ÑÌäº???åÎ??????∏ÌåÖ
		break;
	case IPC_WAITFORACK:	break;
	case IPC_ERROR:		break;
	case IPC_ADDR_SET:	// ?§Ï†ï(&O)Î≤ÑÌäº???åÎ?????
		pSendButton->EnableWindow(TRUE);
		pEnetNameCombo->EnableWindow(FALSE);
		break;
	case IPC_ADDR_RESET: // ?¨ÏÑ§??&R)Î≤ÑÌäº???åÎ?????
		pSetAddrButton->SetWindowText("?§Ï†ï(&O)");
...
void CArpAppDlg::OnTimer(UINT nIDEvent)
{
	...
	//timerÍ∞Ä ?§Ìñâ???åÎßà??1Ï¥? table??Í≤Ä?âÌïò??cache.type==true??entryÎ•??ïÏù∏ ??List Table???àÎã§Î©?Í∏∞Ï°¥ Î¨∏Ïûê?¥ÏùÑ Î≥ÄÍ≤? ?ÜÎã§Î©?Ï∂îÍ???
